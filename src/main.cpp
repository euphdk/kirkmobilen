...
void Dial() {
  // I'm not using Debounce here because for some odd reason it just doesn't
  // work :(

  /* Check that we have received some digits in the number, and the rotary dial
   * hasn't been dialed for ASSUME_NUMBER_FINISHED_AFTER_MS, then transistion to
   * ALERTING */
  if (lastDigitReceivedTime != 0 && (currentMillis - lastDigitReceivedTime) >
                                        ASSUME_NUMBER_FINISHED_AFTER_MS) {
    Serial.println();
  ...
  if ((currentMillis - lastPulseChangeTime) >
      ROTARY_HAS_FINISHED_ROTATING_AFTER_MS) {
    // if it’s only just finished being dialed, we need to send the number down
    // the serial line and reset the pulseCount. We mod the pulseCount by 10
    // because ‘0’ will send 10 pulses.
    if (needToPrint) {
  ...
  if ((currentMillis - lastPulseChangeTime) > DEBOUNCE_DELAY) {
    ...
    if (pulse != truePulseState) {
      // this means that the switch has either just gone from closed->open or
      // vice versa.
...
void loop() {
  ...
  switch (curstate) {
    case IDLE:
      ...
      if ((currentMillis - lastCheckCallStatusTime) >
          PRINT_CALL_STATUS_INTERVAL) {
